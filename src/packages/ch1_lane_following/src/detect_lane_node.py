...
"""
Semantic lane detection with a U-Net, and the cross-track error it implies.
...
The advantage over a classic colour filter is robustness: the network finds the
lane without the illumination being calibrated by hand, so it copes with
changing light.
"""
...
class DetectLaneNode:
    ...
    def __init__(self, node_name):
        ...
        # The raw geometry behind the error. detect_intersection reads the red
        # line fields from it; the rest is published so later challenges can
        # build on it without touching this node.
    ...
    def _cb_mode(self, msg):
        ...
        Standing at a line or driving blind over it, the only lane markings
        that mean anything are directly in front of the wheels. While clearing
        the line the robot is lane following again, so it looks ahead normally.
        """
        try:
        ...
        if mode in (DriveMode.AT_STOP_LINE, DriveMode.CROSSING):
