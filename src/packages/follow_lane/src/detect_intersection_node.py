...
class DetectIntersectionNode:
    def __init__(self, node_name):
        ...
        self.sub_image = rospy.Subscriber(self._camera_topic, CompressedImage, self.cbProcessImage, queue_size=1)
    ...
    def cbProcessImage(self, image_msg):
        ...
        lower_red_upper = np.array([self.hue_red_l, self.saturation_red_l, self.lightness_red_l], dtype=np.uint8)
        upper_red_upper = np.array([self.hue_red_h, self.saturation_red_h, self.lightness_red_h], dtype=np.uint8)
        mask2 = cv2.inRange(hsv, lower_red_upper, upper_red_upper)

        # Lower Red Mask (Hardcoded wraparound for 0-10)
        lower_red_lower = np.array([0, self.saturation_red_l, self.lightness_red_l], dtype=np.uint8)
        upper_red_lower = np.array([10, self.saturation_red_h, self.lightness_red_h], dtype=np.uint8)
